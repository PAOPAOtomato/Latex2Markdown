# back up of the modified markdownify __init__.py

from bs4 import BeautifulSoup, NavigableString, Comment, Doctype
import re
import six

# Regular expressions for headings and whitespace
...
def replace_p_li(html):
    return html.replace('</p></li>', '<br>')
...
class MarkdownConverter(object):
    ...
    def convert_td(self, el, text, convert_as_inline):
        ...
        if 'colspan' in el.attrs and el['colspan'].isdigit():
            ...
            text = f"<COLSPAN={colspan}> {text.strip()}"
        if 'rowspan' in el.attrs and el['rowspan'].isdigit():
            rowspan = int(el['rowspan'])
            text = f"<ROWSPAN={rowspan}> {text.strip()}"
            # for i in range(1, rowspan):
            #     if el.find_parent('tr').next_sibling:
            #         next_row = el.find_parent('tr').next_sibling
            #         next_cell = BeautifulSoup(f'<td></td>', 'html.parser')
    ...
    def convert_th(self, el, text, convert_as_inline):
        ...
        if 'rowspan' in el.attrs and el['rowspan'].isdigit():
            ...
            #     if el.find_parent('tr').next_sibling:
            #         next_row = el.find_parent('tr').next_sibling
            #         next_cell = BeautifulSoup(f'<th></th>', 'html.parser')
            #         next_row.append(next_cell)
        return ' ' + text.strip().replace("\n", " ") + ' |' * colspan
    ...
    def convert_tr(self, el, text, convert_as_inline):
        ...
        # Add empty cells for rowspan
        for cell in cells:
            if 'rowspan' in cell.attrs and cell['rowspan'].isdigit():
                rowspan = int(cell['rowspan'])
                cell_index = el.contents.index(cell)
                for i in range(1, rowspan):
                    if el.find_parent('table').find_all('tr')[el.parent.index(el) + i]:

                        target_row = el.find_parent('table').find_all('tr')[el.parent.index(el) + i]
                        new_cell = el.__class__(name=cell.name)
                        # AA = target_row.find_all('td')[cell_index]
                        # bb= AA.text
                        # for cell in target_row.find_all(bb):
                        #     if bb in cell.text:
                        #         cell.insert(new_cell)
                        #         break

                    # cell_index += 1
                    target_row.insert(cell_index+1, new_cell)
